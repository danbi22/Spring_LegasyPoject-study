...
// 스프링 컨텍스트(application-context.xml 또는 servlet-context.xml)를 사용하지 않는 단위 테스트에서는 
...
public class StreamTest {
    ...
    public void test() {
        ...
        List<Integer> odds = numbers.stream().filter((o) -> o % 2 == 1).toList();
        ...
        List<LocalDateTime> times = Arrays.asList(
                ...
                LocalDateTime.of(2023, 5, 25, 18, 0, 0)
        );
        System.out.println(times);
        
        // LocalDateTime 타입을 Timestamp 타입으로 변환
        List<Timestamp> timestamps = times.stream().map(Timestamp::valueOf).toList();
